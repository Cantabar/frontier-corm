...
- **Ore Optimizer** (`lib/oreOptimizer.ts`) — joint cross-ore optimization for multi-output refining recipes. When multiple ores share intermediate outputs (e.g. both Feldspar and Hydrated Sulfide Matrix produce Hydrocarbon Residue), the tree resolver's DFS byproduct crediting is order-dependent. This module independently re-optimizes ore allocation. `collectRefiningDemands` walks the resolved tree and collects intermediate demands at the refining boundary (craftable nodes with byproducts), stopping before ore-input children. `buildByproductIndex` creates a reverse lookup from output typeId to refining recipes. **Multi-level chain resolution:** `resolveToRawOre` recursively follows primary-output recipes to trace intermediates (e.g. Silica Grains, Hydrocarbon Residue) back to their true raw ore (e.g. Feldspar Crystals, Hydrated Sulfide Matrix). `computeCompoundOutputs` builds per-raw-ore-run yield tables by cascading recipe ratios through the chain — intermediates consumed by the next level are removed, and byproducts at each level are traced through `buildIntermediateInputMap` to convert them into demanded outputs (e.g. Hydrocarbon Residue byproduct from Feldspar refining is converted to Tholin Aggregates output via BP#1186's recipe ratio). This enables cross-chain optimization: Feldspar Crystals runs forced for Feldspar Crystal Shards also produce Tholin via the HR byproduct, reducing Hydrated Sulfide Matrix mining. `optimizeOreUsage` jointly solves for minimum total ore runs using a three-phase algorithm: (1) forced ores — ores that are the sole source for an intermediate get minimum required runs; (2) surplus propagation — all outputs from forced ore runs are credited against remaining demands; (3) greedy fill — remaining shared-intermediate demands are assigned to the most ore-efficient source, iterating until satisfied. `intermediateToOres` is populated from ore model output sets (not the per-demand loop) to capture cross-chain connections. Demand attribution distributes the total demand across ores proportionally to their production for accurate per-ore surplus reporting. SSU inventory is deducted from intermediate demands before optimization. Returns an `OreSummary` with per-ore-type breakdowns and unoptimized (single-output) materials.
- **Blueprint Selection** (`OptimizerPanel`) — when multiple blueprints produce the same output, each craftable tree node shows a dropdown to choose which blueprint/facility to use. Changing a selection re-runs the resolver with the updated recipe. State stored as `Map<outputTypeId, blueprintId>`. **Salvage exclusion:** salvage-input recipes (Salvaged Materials 88764, Mummified Clone 88765) are excluded entirely from both `recipesForOptimizer` and `allRecipesMap` — salvaged materials are too scarce to plan around. Players who refine salvage can place the refined outputs into their SSU, where they will be consumed by the inventory-aware resolver without needing a salvage recipe in the planner.
...
- **Ore Summary** (`OptimizerPanel`) — toggleable "Ore Summary" section below gap analysis. When active, extracts intermediate demands from the resolved tree via `collectRefiningDemands`, then jointly optimizes ore runs across all ore types via `optimizeOreUsage`. Shows minimum ore units to mine per ore type, per-product breakdown (needed vs. produced) with demand attributed per-ore, and surplus byproduct quantities. Cross-ore overlap (e.g. Hydrocarbon Residue from both Feldspar and HSM) is handled by the joint optimizer's surplus propagation, avoiding redundant mining. SSU-aware: on-hand intermediates reduce refining demand.
...
- Network Node metadata URL repair: owner-only "Set Link" action on `NetworkNodeGroup` headers sets or updates the on-chain metadata URL for nodes that were installed before URL-writing was added. Reads current URL from on-chain `Metadata.url` and only shows the action when the URL is missing or outdated. A complementary "Clear Link" action (shown when the URL is synced) lets the owner clear the on-chain URL back to empty by calling `network_node::update_metadata_url` with `""`. The two buttons are mutually exclusive: "Set Link" appears when the URL is absent/stale, "Clear Link" appears when it matches the expected value.
