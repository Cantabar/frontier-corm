...
func NewSession(playerAddress, sessionContext string) *Session {
	s := &Session{
		...
		EventBuffer:         types.NewRingBuffer(256),
		ActionChan:      make(chan types.CormAction, 64),
	}
...
// GetID implements types.ActionTarget.
func (s *Session) GetID() string { return s.ID }

// GetPlayerAddress implements types.ActionTarget.
func (s *Session) GetPlayerAddress() string { return s.PlayerAddress }

// GetContext implements types.ActionTarget.
func (s *Session) GetContext() string { return s.Context }

// GetEventBuffer implements types.ActionTarget.
func (s *Session) GetEventBuffer() *types.RingBuffer { return s.EventBuffer }
...
// --- SessionStoreAdapter satisfies types.SessionLookup ---

// SessionStoreAdapter wraps SessionStore to satisfy types.SessionLookup.
type SessionStoreAdapter struct {
	Store *SessionStore
}

// Get returns a session as a types.ActionTarget.
func (a *SessionStoreAdapter) Get(id string) types.ActionTarget {
	s := a.Store.Get(id)
	if s == nil {
		return nil
	}
...
// All returns all sessions as types.ActionTarget.
func (a *SessionStoreAdapter) All() []types.ActionTarget {
	sessions := a.Store.All()
	out := make([]types.ActionTarget, len(sessions))
