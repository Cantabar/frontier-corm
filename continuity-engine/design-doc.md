...
- **Local (localnet):** built and run via `mprocs.yaml` using air for live-reload
- **Local (testnet):** `make local-stillness` runs `mprocs.stillness.yaml` — sources `.env.stillness` for all package/object IDs, points `SUI_RPC_URL` at `https://fullnode.testnet.sui.io:443`, sets `SEED_CHAIN_DATA=false`, and derives `SUI_PRIVATE_KEY` from the active Sui CLI keypair. Uses local Postgres and air for live-reload.
